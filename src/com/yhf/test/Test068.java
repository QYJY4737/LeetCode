...
public class Test068 {
    ...
    private static void solution(String line) {
        ...
        int[] high = new int[split.length];
        for (int i = 0; i < high.length; i++) {
            try {
                high[i] = Integer.parseInt(split[i]);
            } catch (Exception e) {
                System.out.print("[]");
                return;
            }
        ...
        for (int i = 0; i < high.length - 1; i++) {
            if (i % 2 == 0 && high[i] < high[i + 1]) {
                swap(high, i, i + 1);
            }
            if (i % 2 == 1 && high[i] > high[i + 1]) {
                swap(high, i, i + 1);
            }
        }
        for (int i = 0; i < high.length; i++) {
    ...
    private static void swap(int[] high, int x, int y) {
        int tmp = high[x];
        high[x] = high[y];
        high[y] = tmp;
    }
